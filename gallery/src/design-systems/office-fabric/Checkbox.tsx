...
export const Checkbox: React.FC = () => {
  ...
  return (
    <div className="ds-office-fabric" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <label className="ds-label-office-fabric">Single Checkbox (Interactive)</label>
        <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', marginTop: '6px', fontSize: '14px' }}>
          <input
            type="checkbox"
            className="ds-checkbox-office-fabric"
            checked={singleChecked}
            onChange={e => setSingleChecked(e.target.checked)}
          />
          <span>I agree to the privacy and telemetry compliance terms</span>
        </label>
        <span className="ds-helper-office-fabric">Status: {singleChecked ? 'Consented' : 'Declined'}</span>
      </div>

      <div>
        <label className="ds-label-office-fabric">Checkbox Group (Multiple Selection)</label>
        ...
        <span className="ds-helper-office-fabric">Selected channels: {selectedGroup.length ? selectedGroup.join(', ') : 'None'}</span>
      </div>
      ...
      <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: '14px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' }}>
            <input type="checkbox" className="ds-checkbox-office-fabric" defaultChecked name="chk-demo-office-fabric" />
            <span>Checked</span>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' }}>
            <input type="checkbox" className="ds-checkbox-office-fabric" name="chk-demo-office-fabric" />
            <span>Unchecked</span>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' }}>
            <input type="checkbox" className="ds-checkbox-office-fabric" ref={indRef} />
            <span>Indeterminate</span>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: 0.55 }}>
            <input type="checkbox" className="ds-checkbox-office-fabric" disabled />
            <span>Disabled Unchecked</span>
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: 0.55 }}>
            <input type="checkbox" className="ds-checkbox-office-fabric" defaultChecked disabled />
