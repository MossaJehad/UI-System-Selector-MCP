...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-rhds" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-rhds">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-rhds ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-rhds"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-rhds ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-rhds">
