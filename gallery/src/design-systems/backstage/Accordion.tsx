...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-backstage" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-backstage">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-backstage ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-backstage"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-backstage ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-backstage">
