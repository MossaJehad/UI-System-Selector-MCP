...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-vengeance-ui" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-vengeance-ui">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-vengeance-ui ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-vengeance-ui"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-vengeance-ui ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-vengeance-ui">
