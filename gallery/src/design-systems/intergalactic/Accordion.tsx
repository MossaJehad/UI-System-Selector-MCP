...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-intergalactic" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-intergalactic">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-intergalactic ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-intergalactic"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-intergalactic ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-intergalactic">
