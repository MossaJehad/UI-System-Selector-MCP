...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-semantic" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-semantic">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-semantic ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-semantic"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-semantic ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-semantic">
