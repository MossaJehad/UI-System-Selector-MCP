...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-one-ui" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-one-ui">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-one-ui ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-one-ui"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-one-ui ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-one-ui">
