...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-gestalt" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-gestalt">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-gestalt ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-gestalt"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-gestalt ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-gestalt">
