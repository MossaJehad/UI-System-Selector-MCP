...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-orbit" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-orbit">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-orbit ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-orbit"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-orbit ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-orbit">
