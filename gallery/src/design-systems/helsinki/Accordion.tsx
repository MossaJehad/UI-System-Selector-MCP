...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-helsinki" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-helsinki">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-helsinki ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-helsinki"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-helsinki ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-helsinki">
