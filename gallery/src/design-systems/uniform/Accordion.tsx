...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-uniform" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-uniform">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-uniform ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-uniform"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-uniform ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-uniform">
