...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-salesforce" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-salesforce">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-salesforce ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-salesforce"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-salesforce ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-salesforce">
