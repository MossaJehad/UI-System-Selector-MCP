...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-ubuntu" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-ubuntu">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-ubuntu ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-ubuntu"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-ubuntu ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-ubuntu">
