...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-warcraftcn-ui" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-warcraftcn-ui">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-warcraftcn-ui ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-warcraftcn-ui"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-warcraftcn-ui ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-warcraftcn-ui">
