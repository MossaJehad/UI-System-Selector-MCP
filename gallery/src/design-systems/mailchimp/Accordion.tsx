...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-mailchimp" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-mailchimp">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-mailchimp ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-mailchimp"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-mailchimp ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-mailchimp">
