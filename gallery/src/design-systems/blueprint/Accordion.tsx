...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-blueprint" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-blueprint">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-blueprint ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-blueprint"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-blueprint ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-blueprint">
