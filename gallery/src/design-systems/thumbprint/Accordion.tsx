...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-thumbprint" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-thumbprint">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-thumbprint ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-thumbprint"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-thumbprint ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-thumbprint">
