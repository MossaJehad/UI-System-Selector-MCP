    ...
    <div className="ds-7-css" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
          <label className="ds-label-7-css" style={{ margin: 0 }}>Release Notes</label>
          <span style={{ fontSize: '12px', color: 'var(--fg-muted)' }}>{notes.length} / 500</span>
        </div>
        <textarea
          className="ds-textarea-7-css"
          rows={4}
          placeholder="Document service changelog, migration steps, and deployment sign-offs..."
          value={notes}
          onChange={e => setNotes(e.target.value)}
        />
        <span className="ds-helper-7-css">Markdown formatting supported</span>
      </div>

      <div>
        <label className="ds-label-7-css">Audit Record (Read-Only)</label>
        <textarea
          className="ds-textarea-7-css"
          rows={4}
          disabled
