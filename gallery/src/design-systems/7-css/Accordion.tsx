...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-7-css" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-7-css">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-7-css ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-7-css"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-7-css ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-7-css">
