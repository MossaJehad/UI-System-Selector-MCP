...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-crayons" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-crayons">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-crayons ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-crayons"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-crayons ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-crayons">
