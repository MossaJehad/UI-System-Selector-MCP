...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-momentum" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-momentum">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-momentum ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-momentum"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-momentum ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-momentum">
