...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-hana-ui" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-hana-ui">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-hana-ui ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-hana-ui"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-hana-ui ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-hana-ui">
