...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-pega-cosmos" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-pega-cosmos">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-pega-cosmos ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-pega-cosmos"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-pega-cosmos ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-pega-cosmos">
