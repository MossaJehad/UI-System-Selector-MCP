...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-seeds" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-seeds">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-seeds ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-seeds"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-seeds ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-seeds">
