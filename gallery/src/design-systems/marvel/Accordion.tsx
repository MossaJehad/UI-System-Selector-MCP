...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-marvel" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-marvel">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-marvel ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-marvel"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-marvel ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-marvel">
