...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-acorn" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-acorn">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-acorn ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-acorn"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-acorn ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-acorn">
