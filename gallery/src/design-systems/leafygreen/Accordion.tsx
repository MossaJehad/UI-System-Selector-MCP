...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-leafygreen" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-leafygreen">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-leafygreen ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-leafygreen"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-leafygreen ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-leafygreen">
