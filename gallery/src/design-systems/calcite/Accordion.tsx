...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-calcite" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-calcite">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-calcite ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-calcite"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-calcite ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-calcite">
