...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-servicenow" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-servicenow">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-servicenow ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-servicenow"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-servicenow ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-servicenow">
