...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-xp-css" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-xp-css">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-xp-css ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-xp-css"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-xp-css ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-xp-css">
