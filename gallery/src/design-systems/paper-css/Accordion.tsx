...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-paper-css" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-paper-css">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-paper-css ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-paper-css"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-paper-css ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-paper-css">
