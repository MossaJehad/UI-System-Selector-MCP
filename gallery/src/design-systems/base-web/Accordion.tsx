...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-base-web" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-base-web">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-base-web ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-base-web"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-base-web ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-base-web">
