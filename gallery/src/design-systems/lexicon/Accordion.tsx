...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-lexicon" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-lexicon">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-lexicon ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-lexicon"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-lexicon ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-lexicon">
