...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-liquid" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-liquid">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-liquid ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-liquid"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-liquid ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-liquid">
