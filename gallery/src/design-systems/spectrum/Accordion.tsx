...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-spectrum" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-spectrum">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-spectrum ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-spectrum"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-spectrum ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-spectrum">
