...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-pluralsight" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-pluralsight">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-pluralsight ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-pluralsight"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-pluralsight ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-pluralsight">
