...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-ant" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-ant">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-ant ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-ant"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-ant ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-ant">
