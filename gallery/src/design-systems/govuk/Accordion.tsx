...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-govuk" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-govuk">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-govuk ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-govuk"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-govuk ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-govuk">
