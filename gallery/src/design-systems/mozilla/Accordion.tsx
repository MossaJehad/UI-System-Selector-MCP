...
export const Accordion: React.FC = () => {
  ...
  return (
    <div className="ds-mozilla" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div className="ds-accordion-mozilla">
        {items.map((item, idx) => {
          const isOpen = openIndex === idx;
          return (
            <div key={idx} className={`ds-accordion-item-mozilla ${isOpen ? 'open' : ''}`}>
              <button
                type="button"
                className="ds-accordion-trigger-mozilla"
                aria-expanded={isOpen}
                onClick={() => toggle(idx)}
              >
                <span>{item.title}</span>
                <span className={`ds-accordion-chevron-mozilla ${isOpen ? 'rotated' : ''}`}>▾</span>
              </button>
              {isOpen && (
                <div className="ds-accordion-panel-mozilla">
